...
use reth_eth_wire_types::{Block, BlockHeader, HeadersDirection, NetworkTypes};
use reth_network_peers::WithPeerId;
use reth_primitives::{
    alloy_primitives::{Sealable, Sealed},
    BlockBody, GotExpected, Header, SealedBlock, SealedHeader, B256,
};
use std::{
    cmp::Reverse,
    ...
    Client: BlockClient<T>,
{
    ...
    fn take_block(&mut self) -> Option<SealedBlock> {
        ...
        match resp {
            BodyResponse::Validated(body) => Some(<T::Block as Block>::build_sealed(header, body)),
            BodyResponse::PendingValidation(resp) => {
                // ensure the block is valid, else retry
                if let Err(err) = T::validate_block_body(&header, resp.data()) {
                ...
                Some(<T::Block as Block>::build_sealed(header, resp.into_data()))
            }
        }
    }
    ...
    fn take_blocks(&mut self) -> Option<Vec<SealedBlock>> {
        ...
        for header in &headers {
            if let Some(body_resp) = self.bodies.remove(header) {
                ...
                valid_responses.push(<T::Block as Block>::build_sealed(header.clone(), body));
            }
        }
        ...
        if needs_retry {
            ...
            for block in valid_responses {
                let (header, body) = <T::Block as Block>::split_sealed(block);
