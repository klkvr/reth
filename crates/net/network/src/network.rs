...
use crate::{
    config::NetworkMode, protocol::RlpxSubProtocol, transactions::TransactionsHandle,
    DiscoveryEvent, FetchClient, NetworkConnectionState, NetworkEvent, PeerRequest,
};

/// Helper trait that unifies network API needed to launch node.
pub trait FullNetwork<T: NetworkTypes>:
    BlockDownloaderProvider<T>
    + NetworkSyncUpdater
    + NetworkInfo
    + NetworkEvents<T>
    + PeersInfo
...
impl<T, Types: NetworkTypes> FullNetwork<Types> for T where
    T: BlockDownloaderProvider<Types>
        + NetworkSyncUpdater
        + NetworkInfo
        + NetworkEvents<Types>
        + PeersInfo
...
impl<T: NetworkTypes> NetworkEvents<T> for NetworkHandle<T> {
...
pub trait NetworkEvents<T: NetworkTypes>: Send + Sync {
